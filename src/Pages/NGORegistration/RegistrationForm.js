...
let payload = {
  ...
  orgName: requestType === "subsequent" ? selectedExistingNgo : updatedFormData.organizationName 
};

// If existing NGO selected, send orgName separately
if (requestType === "subsequent" && selectedExistingNgo) {
  const existingOrg = existingNgos.find(org => org.organizationName === selectedExistingNgo);
  if (existingOrg) {
    ...
    <Button
      onClick={() => {
        if (requestType === "first-time") {
          setRequestType("first-time"); // ensure correct value
          setShowRequestTypeModal(false);
        } else if (requestType === "subsequent" && selectedExistingNgo) {
          setRequestType("subsequent"); // ensure correct value
          setFormData(prev => ({
            ...prev,
