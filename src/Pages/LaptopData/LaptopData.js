...
export default function LaptopData() {
    ...
    const handleInputChange = (e) => {
        ...
        const filteredItems = filteredData1.filter((user) =>
            ...
            user.userId?.toString().includes(searchTerm) || // Check userId (convert to string)
            user.Address?.email?.toLowerCase().includes(searchTerm.toLowerCase()) || // Check Address.email (with optional chaining)
            user['contact number']?.toString().includes(searchTerm) // Check contact number (convert to string)
        );
